...
async function handleTextMessage(event, user) {
  ...
  try {
    ...
    const parsedMetrics = parseBodyMetricsInput(text);
    const parsedWeight = parsedMetrics.weightKg;
    const parsedBodyFat = parsedMetrics.bodyFatPercent;
    ...
    if (parsedWeight !== null || parsedBodyFat !== null) {
      ...
      if (parsedBodyFat !== null) {
        await saveBodyFatToLog(user.id, parsedBodyFat, text);
      }
      ...
      if (parsedWeight !== null && parsedBodyFat !== null) {
        lines.push('体重と体脂肪率を保存しました。');
        lines.push(`体重: ${parsedWeight}kg`);
        lines.push(`体脂肪率: ${parsedBodyFat}%`);
      } else if (parsedWeight !== null) {
        lines.push('体重を保存しました。');
        lines.push(`今回: ${parsedWeight}kg`);
      } else if (parsedBodyFat !== null) {
        lines.push('体脂肪率を保存しました。');
        lines.push(`今回: ${parsedBodyFat}%`);
      }
      ...
      if (parsedBodyFat !== null && parsedWeight === null) {
        lines.push('体重も分かれば、続けてそのまま送ってください。');
      }
