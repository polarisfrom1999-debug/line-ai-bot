...
/**
 * ここから。LINE AI Bot - Stable Unified Edition
 * - OpenAI: 相談 / 食事画像解析
 * - Supabase: user_logs, user_profiles, body_records, meal_records
 * - Graph: canvas + chart.js (optional). 無ければテキストで返す
 * - Safety: medical risk check, prompt injection sanitize, rate limit, self-lint
 * - LINE: static /public 配信で画像返信
 */
...
// ===== optional deps: canvas/chart.js (無ければテキストのみ) =====
let createCanvas = null;
let Chart = null;
try {
  ({ createCanvas } = require("canvas"));
  Chart = require("chart.js/auto");
  console.log("[OK] canvas/chart.js loaded -> graph enabled");
} catch (e) {
  console.warn("[WARN] canvas/chart.js not available -> graph fallback to text");
}
...
// サーバー側は service_role 推奨（RLSを使うなら関数やRLS設計とセットで）
// ※ANONでやる場合はRLS/ポリシーを必ず調整してください。
...
try {
  ...
  if (!isHttps(BASE_URL)) {
    throw new Error("BASE_URL must start with https:// (LINE image requires public https URL)");
  }
} catch (e) {
...
// ===== self-lint: “同じ過ちを繰り返さない” =====
// コピペ事故で「Authorization: Bearer ${...}」や「url: data:image...${...}」が残ると即死する
function selfLintOrExit() {
  try {
    const self = fs.readFileSync(__filename, "utf8");
    const badPatterns = [
      // Bearer のテンプレ崩れ（スペース/改行が入っても拾う）
      { re: /Authorization:\s*Bearer\b[\s]*\$\{/g, msg: "Found broken Authorization header: use `Bearer ${TOKEN}`" },
      // data:image のテンプレ崩れ
      { re: /url:\s*data:image\/(jpeg|png);base64,\$\{/g, msg: "Found broken data:image URL: use `data:image/jpeg;base64,${...}`" },
    ];
    const hits = badPatterns.filter((p) => p.re.test(self));
    if (hits.length) {
      console.error("❌ SELF-LINT FAILED. Fix these patterns:");
...
function lineAuthHeader() {
  // GET/画像取得など Content-Type不要
  return { Authorization: Bearer ${LINE_CHANNEL_ACCESS_TOKEN} };
}
...
function predictFutureWeight3m(currentWeight) {
  ...
  return +(currentWeight - 2).toFixed(1); // シンプル予測（後で強化）
}
