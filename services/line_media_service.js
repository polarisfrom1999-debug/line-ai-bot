...
function buildLineBlobClient() {
  ...
  try {
    return new line.messagingApi.MessagingApiBlobClient({ channelAccessToken: token });
  } catch (_) {
    return null;
  }
}
...
async function getMessageContentBuffer(messageId) {
  ...
  try {
    ...
    if (response?.readable === true && typeof response?.on === 'function') return await streamToBuffer(response);
    if (response?.data?.readable === true && typeof response.data?.on === 'function') return await streamToBuffer(response.data);
    return null;
  } catch (error) {
...
async function getImagePayload(input) {
  const messageId = input?.imageMeta?.messageId || input?.messageId || input?.originalEvent?.message?.id || null;
  if (!messageId) return null;
  const buffer = await getMessageContentBuffer(messageId);
  if (!buffer || !buffer.length) return null;
  return { messageId, buffer, mimeType: 'image/jpeg', size: buffer.length };
}

module.exports = { getImagePayload, getMessageContentBuffer };
