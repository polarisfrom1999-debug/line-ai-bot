...
const { buildEnergySummaryText } = require('./services/energy_service');
const {
...
const {
  ...
  isVagueReference,
  isLostIntent,
  buildLostGuideMessage,
  buildYesNoFollowup,
  buildVagueReferenceHelp,
} = require('./services/conversation_repair_service');
const {
...
const { safeText, fmt } = require('./utils/formatters');
const { toIsoStringInTZ, currentDateYmdInTZ } = require('./utils/dates');
...
async function getRecentWeightLogs(userId, limit = 14) {
...
async function saveWeightLog(user, parsed) {
...
async function handleTextMessage(event, user) {
  ...
  try {
    ...
    if (isLostIntent(text)) {
      const msg = buildLostGuideMessage();
      await replyMessage(
        event.replyToken,
        textMessageWithQuickReplies(prefixWithName(user, msg.text), msg.quickReplies),
        env.LINE_CHANNEL_ACCESS_TOKEN
      );
      return;
    }

    if (isVagueReference(text)) {
      const msg = buildVagueReferenceHelp();
      await replyMessage(
        event.replyToken,
        textMessageWithQuickReplies(prefixWithName(user, msg.text), msg.quickReplies),
        env.LINE_CHANNEL_ACCESS_TOKEN
    ...
    if (currentMealDraft && isSimpleYes(text)) {
      const yesNo = buildYesNoFollowup('meal');
      await replyMessage(
        event.replyToken,
        textMessageWithQuickReplies(prefixWithName(user, yesNo.yes.text), yesNo.yes.quickReplies),
        env.LINE_CHANNEL_ACCESS_TOKEN
      );
      return;
    }
    if (currentMealDraft && isSimpleNo(text)) {
      const yesNo = buildYesNoFollowup('meal');
      await replyMessage(
        event.replyToken,
        textMessageWithQuickReplies(prefixWithName(user, yesNo.no.text), yesNo.no.quickReplies),
        env.LINE_CHANNEL_ACCESS_TOKEN
      );
      return;
    }

    if ((supportContext?.mode === 'pain' || supportContext?.mode === 'stretch') && isSimpleYes(text)) {
      const yesNo = buildYesNoFollowup('pain');
      await replyMessage(
        event.replyToken,
        textMessageWithQuickReplies(prefixWithName(user, yesNo.yes.text), yesNo.yes.quickReplies),
        env.LINE_CHANNEL_ACCESS_TOKEN
    ...
    if ((supportContext?.mode === 'pain' || supportContext?.mode === 'stretch') && isSimpleNo(text)) {
      const yesNo = buildYesNoFollowup('pain');
      await replyMessage(
        event.replyToken,
        textMessageWithQuickReplies(prefixWithName(user, yesNo.no.text), yesNo.no.quickReplies),
    ...
    if (isWeightIntent(text)) {
      const parsed = parseWeightLog(text);
      if (parsed) {
        await saveWeightLog(user, parsed);
        const msg = buildWeightSaveMessage(parsed);
        await replyMessage(
          event.replyToken,
          textMessageWithQuickReplies(prefixWithName(user, msg.text), msg.quickReplies),
          env.LINE_CHANNEL_ACCESS_TOKEN
    ...
    if (text === '体重グラフ') {
      const rows = await getRecentWeightLogs(user.id, 14);
      const graph = buildWeightGraphMessage(rows);
      const messages = [textMessageWithQuickReplies(prefixWithName(user, graph.text), ['予測', '食事活動グラフ', '血液検査グラフ', '今日はここまで'])];
    ...
    if (text === '食事活動グラフ') {
      ...
      const messages = [textMessageWithQuickReplies(prefixWithName(user, graph.text), ['予測', '体重グラフ', '血液検査グラフ', '今日はここまで'])];
      if (graph.messages.length) messages.push(...graph.messages);
      await replyMessage(event.replyToken, messages, env.LINE_CHANNEL_ACCESS_TOKEN);
      return;
    }
    ...
    if (text === '血液検査グラフ' || text === 'HbA1cグラフ' || text === 'LDLグラフ') {
      ...
      const messages = [textMessageWithQuickReplies(prefixWithName(user, graph.text), ['体重グラフ', '食事活動グラフ', '予測'])];
      if (graph.messages.length) messages.push(...graph.messages);
      await replyMessage(event.replyToken, messages, env.LINE_CHANNEL_ACCESS_TOKEN);
      return;
    }
    ...
    if (text === '血液検査の流れを見たい') {
      ...
      const messages = [textMessageWithQuickReplies(prefixWithName(user, graph.text), ['LDLグラフ', '食事活動グラフ', '体重グラフ', '予測'])];
      if (graph.messages.length) messages.push(...graph.messages);
      await replyMessage(event.replyToken, messages, env.LINE_CHANNEL_ACCESS_TOKEN);
      return;
    }
    ...
    const currentMealDraft2 = getMealDraft(user.line_user_id);

    if (currentMealDraft2 && isMealSaveCommand(text)) {
      const savedMeal = await saveMealToLog(user.id, currentMealDraft2.meal);
      clearMealDraft(user.line_user_id);
    ...
    if (currentMealDraft2 && isMealCancelCommand(text)) {
      clearMealDraft(user.line_user_id);
      await replyMessage(event.replyToken, '食事の確認中データを取り消しました。', env.LINE_CHANNEL_ACCESS_TOKEN);
      return;
    }

    if (currentMealDraft2 && seemsMealCorrectionText(text)) {
      const correctedMeal = await applyMealCorrection(currentMealDraft2.meal, text);
