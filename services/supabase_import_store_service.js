...
function createSupabaseImportStore(supabase) {
  ...
  async function resolveInternalUser(lineOrUserId) {
    const safe = normalizeText(lineOrUserId);
    if (!safe) throw new Error('user id is required');
    if (isUuid(safe)) return { id: safe, line_user_id: null };
    return ensureUser(supabase, safe);
  }

  return {
    async createSession({ domain, userId, sourceType, mediaCount, sessionMeta }) {
      const user = await resolveInternalUser(userId);
      const res = await supabase
        ...
        .insert({
          ...
          session_meta: {
            ...(sessionMeta || {}),
            line_user_id: user.line_user_id || (!isUuid(normalizeText(userId)) ? normalizeText(userId) : null),
          },
        })
    ...
    async saveNormalizedFacts({ sessionId, userId, domain, normalized, rawId }) {
      ...
      if (domain === 'lab') {
        for (const measurement of normalized?.measurements || []) {
          const numericValue = typeof measurement.value === 'number' ? measurement.value : Number(measurement.value);
          facts.push({
            session_id: sessionId,
            user_id: user.id,
            domain,
            ...
            fact_value_num: Number.isFinite(numericValue) ? numericValue : null,
