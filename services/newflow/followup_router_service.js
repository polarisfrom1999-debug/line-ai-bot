...
function inferDomainFromText(text) {
  ...
  if (/(TG|LDL|HDL|HbA1c|中性脂肪|検査|患者|氏名|クリニック|病院|採血|日付|悪い|何が|読め|印刷|異常|悪|値|H\/L|変化|推移|傾向|対策|気をつける|他の日付|他の日は|他の検査日|何日分|前回より|前回と比|前回と比較|高い数値|低い数値|バランス|全部教えて|保存項目)/i.test(safe)) {
    return 'lab';
  }
