...
function buildLabGenericReply() {
  return '今確認できる範囲では、検査画像の続きとして扱います。項目名（例: TG, LDL, HbA1c）を指定して聞いてください。';
}
...
function buildTodayBalanceReply(b) {
  if (!b) {
    return '⚖️ 日次の収支をいま集計できませんでした。少し待ってからもう一度送ってください。';
  }
  const d = b.dateYmd || '';
  ...
  const tail =
    n > 0
      ...
      : n < 0
        ? '活動消費の方が大きい状態です。'
        : 'ぴったり釣り合っています。';
  return `⚖️ 今日（${d}、東京日付）の収支は、食事（再計算後）の摂取が約${b.intakeKcal} kcal、活動消費が約${b.activityKcal} kcal（活動${b.activityCount}件）で、差分（摂取−活動）が約${b.netKcal} kcal です。${tail}合計補正イベント${b.totalCorrectionEventCount}件。`;
}

function buildTodayIntakeAskReply(b) {
  if (!b) {
    return '🍽️ 日次摂取をいま集計できませんでした。少し待ってからもう一度送ってください。';
  }
  if (!b.mealCount) {
    return '🍽️ 今日（東京日付）の食事はまだ記録がありません。0 kcal 相当です。';
  }
  return `🍽️ 今日、食事は${b.mealCount}件あって、摂取（再計算反映後）の合計は約${b.intakeKcal} kcal（P${b.protein}／F${b.fat}／C${b.carbs}）です。補正${b.totalCorrectionEventCount}件分を数えています。`;
}
...
function buildWeekBalanceReply(w) {
  if (!w) {
    return '⚖️ 週の収支をいま集計できませんでした。少し待ってからもう一度送ってください。';
  }
  const t = w.totals;
  const { good, watch } = buildWeekSoftTips(w);
  const range = `（${w.fromYmd}〜${w.toYmd}、7日分）`;
  const main = `⚖️ 今週の収支${range}は、摂取（食事再計算後）の合計 約${t.weekIntakeKcal} kcal、活動消費（記録上）の合計 約${t.weekActivityKcal} kcal、差分（摂取−活動） 約${t.weekNetKcal} kcal です。補正イベント合計 ${t.weekCorrectionEventCount} 件。`;
  const extra = [good];
  if (watch) {
    extra.push(watch);
  }
...
function buildWeekIntakeAskReply(w) {
  if (!w) {
    return '🍽️ 週の食事分をいま集計できませんでした。少し待ってからもう一度送ってください。';
  }
  const t = w.totals;
  const a = w.averages;
  const { good, watch } = buildWeekSoftTips(w);
  const range = `（${w.fromYmd}〜${w.toYmd}、7日分）`;
  const main = `🍽️ 今週${range}、食事（再計算反映後）の合計摂取は 約${t.weekIntakeKcal} kcal です。食事の件数合計 ${t.weekMealCount} 件、補正イベント合計 ${t.weekCorrectionEventCount} 件。1日平均の摂取は 約${a.avgIntakeKcal} kcal です。`;
  const extra = [good];
  if (watch) {
    extra.push(watch);
  }
