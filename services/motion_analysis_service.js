...
function buildReplyText(result) {
  ...
  if (result.performanceBenefits.length) {
    lines.push('');
    lines.push('続けるとつながること');
...
async function analyzeMotionImage({ imagePayload, imagePayloads, textHint = '', userId = '' } = {}) {
  ...
  const energyLevel = inferEnergyLevel(textHint);
  const prompt = [
    buildSystemPrompt({ timeOfDay, energyLevel }),
    '次の画像について解析し、指定スキーマに厳密に従った JSON のみで返してください。',
    `ユーザー文脈: ${normalizeText(textHint) || '未指定'}`,
    '強みは必ず1つ以上入れる。修正案は最大2個。医療断定は禁止。'
  ].join('\n\n');

  try {
    const result = await geminiService.generateContentJson({
      prompt,
      imagePayloads: payloads,
      schema: getSchema(),
      model: process.env.GEMINI_MOTION_MODEL || 'gemini-2.5-flash-lite',
      temperature: 0.2,
    ...
    if (!normalized.isMotionRelated) {
      return { ...normalized, replyText: 'この画像は動作解析の対象としては読み取り切れませんでした。正面・側面・後面のいずれかが分かる画像だと見やすくなります。' };
    }
    ...
    console.error('[motion_analysis_service] analyzeMotionImage error:', error?.message || error, { userId });
    return buildFallbackResult(textHint);
  }
}
