...
function buildLabExtractPrompt(meta = {}) {
  ...
  const schema = {
    ...
    properties: {
      ...
      rawColumnDates: { type: 'array', items: { type: 'string' } },
      rejectedColumnDates: { type: 'array', items: { type: 'string' } },
      futureAgainstPrintDateCandidates: { type: 'array', items: { type: 'string' } },
  ...
  const prompt = [
    ...
    '画像上部の「印刷日」「報告日」「出力日」は printDate のみ。printDate を observedDate や検査日列として絶対に使わないでください。',
    '検査日 observedDate は、表の列見出しにある YY/MM/DD または YYYY/MM/DD 由来の日付のみ。それ以外から推測した日付を付けないでください。',
    '「基準値」「基準範囲」「正常値」「参考値」「単位」「判定」「H/L」列は検査日列ではない。項目名の直後にある基準範囲は values[] に入れないでください。',
    '基準範囲は rows[].referenceRange に文字列で入れる（例: 6.5-8.2）。values[].value には数値の実測のみ。',
    '6.5-8.2 / 11-35 / 30-149 / 0.61-1.04 のような「数値-数値」範囲は検査値 value ではない。values[] に入れない。',
    'YY/MM/DD は西暦に正しく直す（例: 16/02/25 → 2016-02-25、25/03/22 → 2025-03-22）。',
    'MM/DD のように年が画像に無い場合、printDate の年を勝手に付けて observedDate を完成させないでください。',
    'observedDate が printDate より未来になる場合は、その列日付を確定検査日として採用しない。必要なら optional の futureAgainstPrintDateCandidates に候補だけ列挙（本流の columnDates には入れない）。',
    ...
    '任意デバッグ: rawColumnDates / rejectedColumnDates / futureAgainstPrintDateCandidates に分類メモを入れてよい。columnDates は検査日として確信できる string[] のみ。',
