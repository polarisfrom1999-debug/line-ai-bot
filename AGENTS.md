...
- 陸上日誌ではなく、**選手・親・トレーナー**の三者伴走（WEB は同一タブ内のサブビューで役割別に切替）。
...
| 2 | 親向け要約・日次/月1（WEB は陸上専用のためトレーナー・検査・印刷 UI は出さない） | **実装済**（API は `routes/web.js` に残存可） |
| 3 | レース前後対話・成功体験・リマインド（WEB は上記に集約） | **実装済** |
| 4 | 月1印刷 HTML（API）は残すが、WEB タブからは導線を外す方針 | **API 残** |
...
既定は **OFF**（`services/conversation_surface_service.js`）。自動の言い換えを再度有効にする場合のみ `KOKOKARA_SURFACE_LAYER_ON=1` を設定。
