...
function dedupeLabRows(rows) {
  const seen = new Set();
  return sortByDateAsc(rows).filter((row) => {
...
function panelItemsToHistory(items, examDate) {
  return (Array.isArray(items) ? items : []).map((item) => ({
    ...normalizeLabPanelItem(item),
    history: examDate && item?.value
      ? dedupeLabRows([...(item?.history || []), { date: examDate, value: item.value, unit: item.unit || '', flag: item.flag || '' }])
      : dedupeLabRows(item?.history || [])
  }));
}
...
async function upsertLabPanel(userId, panel) {
  ...
  const source = normalizeString(panel.source || 'image');
  const panelCandidates = [];

  if (Array.isArray(panel.panels) && panel.panels.length) {
    for (const panelRow of panel.panels) {
      const examDate = normalizeString(panelRow.examDate || '');
      const items = panelItemsToHistory(panelRow.items || [], examDate)
        .filter((item) => item.itemName && item.history.length);
      if (examDate && items.length) {
        panelCandidates.push({ examDate, source, capturedAt: nowIso(), items });
      }
    }
  }

  const fallbackExamDate = normalizeString(panel.examDate || '');
  const fallbackItems = panelItemsToHistory(panel.items || [], fallbackExamDate)
    .filter((item) => item.itemName && item.history.length);

  if (!panelCandidates.length && fallbackItems.length) {
    panelCandidates.push({ examDate: fallbackExamDate, source, capturedAt: nowIso(), items: fallbackItems });
  }

  if (!panelCandidates.length) return null;

  for (const normalizedPanel of panelCandidates) {
    const examDate = normalizeString(normalizedPanel.examDate || '');
    let merged = false;
    for (let i = 0; i < current.length; i += 1) {
      ...
      if (examDate && existing.examDate === examDate) {
        ...
        for (const item of normalizedPanel.items) {
          ...
          const history = dedupeLabRows([
            ...(before.history || []),
            ...(item.history || []),
            ...(examDate && item.value ? [{ date: examDate, value: item.value, unit: item.unit || '', flag: item.flag || '' }] : [])
          ].filter((row) => row.date && row.value));
          const latest = history.slice(-1)[0] || null;
          map.set(key, {
            itemName: item.itemName,
            value: latest?.value || item.value || before.value,
            unit: latest?.unit || item.unit || before.unit,
            flag: latest?.flag || item.flag || before.flag,
            history
          });
        }
        current[i] = {
  ...
  return clone(panelCandidates.slice(-1)[0] || null);
}
