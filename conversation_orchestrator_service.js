...
function simplifyMenuText(value) {
...
function buildCompactMealLabel(parsedMeal) {
  const items = Array.isArray(parsedMeal?.items) ? parsedMeal.items.map((item) => simplifyMenuText(item)).filter(Boolean) : [];
  if (!items.length) return '食事';

  if (items.length === 1) return items[0];

  const primary = items[0];
  const sideKeywords = /(定食|丼|プレート|セット|弁当|カレー|ラーメン|うどん|そば|パスタ|サンド|サンドイッチ|バーガー|たこ焼き|鍋|スープ|味噌汁|サラダ)/;
  const sideItems = items.slice(1).filter((item) => !sideKeywords.test(item)).slice(0, 2);

  if (/定食|セット|プレート|弁当/.test(primary)) return primary;
  if (!sideItems.length) return items.slice(0, 2).join('、');
  return `${primary}＋${sideItems.join('・')}`;
}

function buildMealReply(parsedMeal) {
  const items = buildCompactMealLabel(parsedMeal);
  ...
  if ((imageKind === 'menu_text' || imageKind === 'food_package') && !kcal) {
    return [
      `食事候補として ${items} を読み取りました。`,
      parsedMeal?.ocrText ? `読めた文字: ${parsedMeal.ocrText.slice(0, 80)}` : null,
      '実際に食べたものが分かれば、その分だけ栄養までつなげられます。'
    ].filter(Boolean).join('\n');
  }

  const closing = imageKind === 'meal_photo'
