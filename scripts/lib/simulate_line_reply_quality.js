...
function evaluateReplyQuality({
  ...
  if (intentType === 'lab_comparison') {
    if (!/(比較|前回|直近|並べ)/.test(String(reply || ''))) {
      violations.push('lab_comparison_no_anchor');
    }
    if (/(患者名|医療機関|印刷日)/.test(reply)) violations.push('lab_comparison_meta_bloat');
    if (/手入力の目安|今日の合計/.test(reply)) violations.push('lab_meal_bleed');
    return violations;
  }
